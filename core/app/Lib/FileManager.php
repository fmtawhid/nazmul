    ...
    */
	protected function uploadImage(){
        try {
            ...
            // If image processing fails, try to move the file directly as fallback
            try {
                $this->file->move($this->path, $this->filename);
            } catch (\Exception $moveExp) {
                throw new \Exception('Image processing failed: ' . $e->getMessage() . '. Fallback also failed: ' . $moveExp->getMessage());
            }
        }
	}
